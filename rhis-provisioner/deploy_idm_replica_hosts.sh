...
echo "Using Satellite to provision IdM Replica hosts defined in list idm_replica_hosts"
GREEN='\033[0;32m'
...
usage() {
            echo "Usage: deploy_idm_replica_hosts.sh [options]"
            echo "This helper script launches the play to have Satellite deploy the IdM Replica host instances"
            echo "Options:"
...
ansible-playbook --inventory $inventory \
                 ...
                 --extra-vars "platform_hosts={{ idm_replica_hosts }}" \
                 --limit=provisioner \
