...
echo "Using Satellite to deploy satellite capsule hosts defined in list kvm_hypervisor_hosts"
GREEN='\033[0;32m'
...
usage() {
            echo "Usage: deploy_kvm_hypervisors.sh [options]"
            echo "This helper script launches the play to have Satellite deploy new KVM hypervisor host instances"
            echo "Options:"
...
ansible-playbook --inventory $inventory \
                 ...
                 --extra-vars "platform_hosts={{ kvm_hypervisor_hosts }}" \
                 --limit=provisioner \
