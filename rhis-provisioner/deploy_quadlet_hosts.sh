...
echo "Using Satellite to provision Quadlet hosts defined in list quadlet_hosts"
GREEN='\033[0;32m'
...
usage() {
            echo "Usage: deploy_quadlet_hosts.sh [options]"
            echo "This helper script launches the play to have Satellite deploy the quadlet host instances"
            echo "Options:"
...
ansible-playbook --inventory $inventory \
                 ...
                 --extra-vars "platform_hosts={{ quadlet_hosts }}" \
                 --limit=provisioner \
