...
echo "Using Satellite to build Host Group test hosts defined in list hostgroup_test_hosts_rhel8"
...
usage() {
            echo "Usage: deploy_rhel8_test_hosts.sh [options]"
            echo "This helper script launches the play to have Satellite deploy test instances for ALL RHEL8 Hostgroups!"
            echo "Options:"
...
ansible-playbook --inventory $inventory \
                 ...
                 --extra-vars "platform_hosts={{ hostgroup_test_hosts_rhel8 }}" \
                 --limit=provisioner \
